...
Should I bring an umbrella in Hong Kong?
```
...
```mermaid
flowchart LR
    U[User question] --> Q[Local Qwen classifier via mimOE]
    Q --> R[Parse structured JSON]
...
The tests use realistic HTTP responses without calling external services. They verify Qwen-based semantic routing, umbrella-question recognition, missing-location behavior, weather lookup, and the exact OpenAI-compatible requests sent to mimOE.
...
- **Semantic routing:** Qwen classifies intent locally, so indirect questions such as "Should I bring an umbrella?" do not depend on a fixed keyword list.
- **Classification fallback:** if Qwen does not return the requested JSON shape, a small deterministic keyword check fails safely and keeps routing predictable.
